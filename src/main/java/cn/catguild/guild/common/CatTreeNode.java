...
import cn.catguild.guild.utility.ObjectTools;
...
public interface CatTreeNode<T, R> {
	...
	static <U extends CatTreeNode<U, Y>, Y> List<U> merge(List<U> allNode) {
		...
		for (U node : allNode) {
			...
			if (node.getParentId() instanceof Long) {
				isTop = ObjectTools.nullSafeEquals(node.getParentId(), 0L);
			}
			if (node.getParentId() instanceof Integer) {
				isTop = ObjectTools.nullSafeEquals(node.getParentId(), 0);
			}
			if (isTop || ObjectTools.nullSafeEquals(node.getParentId(), 0)) {
				// 根节点
	...
	private static <U extends CatTreeNode<U, ?>> List<U> collectChildren(U rootNode, List<U> allNode) {
		...
		for (U node : allNode) {
			if (ObjectTools.nullSafeEquals(node.getParentId(), rootNode.getId())) {
				children.add(node);
				node.setChildren(collectChildren(node, allNode));
			}
		}
