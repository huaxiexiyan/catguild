...
public class Snowflake implements Serializable, IdGenerator{
    ...
    public Snowflake(long workerId, long datacenterId, boolean isUseSystemClock) {
        ...
        if (datacenterId > maxDatacenterId || datacenterId < 0) {
            throw new IllegalArgumentException("datacenter Id can't be greater than %d or less than 0".formatted(maxDatacenterId));
        }
    ...
    public synchronized long nextId() {
        ...
        if (timestamp < lastTimestamp) {
            // 如果服务器时间有问题(时钟后退) 报错。
            throw new IllegalStateException("Clock moved backwards. Refusing to generate id for %dms".formatted(lastTimestamp - timestamp));
        }
