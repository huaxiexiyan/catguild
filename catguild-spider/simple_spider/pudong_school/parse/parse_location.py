...
def load_excel():
    ...
    for row in range(2, worksheet.max_row + 1):
        ...
        school_location = load_location(column_value)
    ...
    # 保存修改后的Excel文件
    workbook.save('../data/学校情况填充表2.xlsx')
    workbook.close()


def load_location(excel_school_name):
    db = get_mongo_client()
    collection = db["school_location"]
    school_location = collection.find_one({"excel_school_name": excel_school_name})
