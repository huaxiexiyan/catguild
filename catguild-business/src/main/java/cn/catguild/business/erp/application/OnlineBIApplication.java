...
public class OnlineBIApplication {
    ...
    private static List<LocalDate> getHorizontalAxis(TimeScale timeScale) {
    ...
    public StatisticsChart salesLineChart(Long tenantId, TimeScale timeScale, StatisticsType type) {
        ...
        lineDayData.forEach(kv -> {
            ...
            total.setSalesRevenue(total.getSalesVolume().add(v.getSalesVolume()));
            total.setSalesRevenue(total.getOrderQuantity().add(v.getOrderQuantity()));
        });
            ...
            lineDayData.forEach(kv -> {
                if (x.isEqual(kv.getK())) {
                    ...
                    switch (type) {
                        ...
                        case ORDER_QUANTITY -> value = kv.getV().getOrderQuantity();
